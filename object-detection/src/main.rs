...
pub fn main() {
    // Parse arguments
    let directory = std::env::args().nth(1).expect("No image path provided");
    ...
    for img_path_new in walkdir::WalkDir::new(directory)
        .into_iter()
        .filter_map(|e| e.ok())
        .filter(|f| f.file_type().is_file())
    {
        let start = Instant::now();
        let img_path = img_path_new.path();
        // Load image
        dbg!(img_path);
        ...
        println!("Time to open image {:?}", start.elapsed());

        // Resize to 640x640
        let resized_img = img.resize_exact(
            WIDTH as u32,
            HEIGHT as u32,
            image::imageops::FilterType::Nearest, // also known as bilinear in 2D
        );

        println!("Image Resizing complete {:?}", start.elapsed());
        ...
        println!("Time to process model {:?}", opening.elapsed());
